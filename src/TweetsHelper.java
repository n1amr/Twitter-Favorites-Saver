...
public class TweetsHelper {
  ...
  public static JSONArray sortTweets(JSONArray tweets,
    ...
    // Copy to array list
    ArrayList<JSONObject> list = JSONHelper.jsonArrayToJSONObjectsList(tweets);
  ...
  public static void saveTweet(JSONObject tweet, boolean override)
    ...
    if (oldTweet == null || override)
      ...
      System.out.println("Skipped; This tweet has already been saved #" + id);
    }
  ...
  static void deleteTweetImage(JSONObject tweet)
    ...
    for (int i = 0; i < mediaArray.length(); i++) {
      ...
      File file = new File(FileHelper.mediaFolder,
          media.getString("media_url").substring(
    ...
    if (file.exists()) {
      ...
      for (JSONObject tweet2 : allTweets) {
        ...
        String filename2 = url2.substring(
            Math.max(url2.lastIndexOf("/"), url2.lastIndexOf("\\")) + 1);
        if (filename2.equals(file.getName())
            && !tweet.getString("id_str").equals(tweet2.getString("id_str")))
          return;
      }
  ...
  static JSONObject fixNewLines(JSONObject tweet) throws Exception {
    ...
    if (text.contains("\n")) {
      ...
      // fix entities
      entities.put("hashtags",
          fixEntity(entities.getJSONArray("hashtags"), replacesIndices));
      entities.put("symbols",
          fixEntity(entities.getJSONArray("symbols"), replacesIndices));
      entities.put("urls",
          fixEntity(entities.getJSONArray("urls"), replacesIndices));
      entities.put("user_mentions",
          fixEntity(entities.getJSONArray("user_mentions"), replacesIndices));
  ...
  static JSONObject undoFixNewLines(JSONObject tweet) throws Exception {
    ...
    if (text.contains("<br>")) {
      ...
      // fix entities
      entities.put("hashtags",
          undoFixEntity(entities.getJSONArray("hashtags"), replacesIndices));
      entities.put("symbols",
          undoFixEntity(entities.getJSONArray("symbols"), replacesIndices));
      entities.put("urls",
          undoFixEntity(entities.getJSONArray("urls"), replacesIndices));
